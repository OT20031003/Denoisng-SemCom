...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        default=f"outputs/MIMOdiffusion/dynamic"
    )
    parser.add_argument(
        "--nosample_outdir",
        type=str,
        nargs="?",
        help="dir to write results to",
        default=f"outputs/jointdiffusion/nonoisenosample/dynamic"
    )
    ...
    q_real_data = z/ torch.sqrt(z_variance + eps).view(-1, 1, 1, 1)
    # チャネル符号化
    ...
    for snr in range(-5, 10, 1):
        ...
        # 非正規化
        z = q_real_data_restored * torch.sqrt(z_variance + eps).view(-1, 1, 1, 1)
        ...
        samples = sampler.MIMO_decide_starttimestep_ddim_sampling(S=opt.ddim_steps, batch_size=batch_size,
                        ...
                        conditioning=cond,starttimestep=None, noise_variance = 1.0 / rho_ul)
