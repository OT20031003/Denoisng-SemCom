        ...
        for s in range(100, 700, 100):
            ...
            if opt.intermediate_path != None:
                for si, samples in past_samples.items():
                    step, index = si
                    if index % opt.intermediate_skip == 0:
                        recoverd_img = model.decode_first_stage(samples)
                        #default=f"./intermediate/k={k}/snr/imgid/starttimestep/step_{}.png",
                        batch_size = recoverd_img.shape[0]
                        for b in range(batch_size):
                            intermediate_dir = os.path.join(opt.intermediate_path, str(snr), str(b), str(s))
                            os.makedirs(intermediate_dir, exist_ok=True)
                            intermediate_file = f"step_{step:04d}.png"
                            img_path = os.path.join(intermediate_dir, intermediate_file)
                            vutil.save_image(recoverd_img[b], img_path)
